...
def title_ner():
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for i in range(302):
            newslist = []
            m = i + 274
            print("title ner for day %d" % m)
            f = open('newsLists/%d.txt' % m, 'r')
            for line in f.readlines():
                newslist.append(int(line))
            fout = codecs.open('title_ner/%d.json' % m, 'w', encoding='utf-8')
            for i in range(len(newslist)):
                day = newslist[i]
                f = codecs.open('D:/PycharmProjects/news_data/%d.json' % day, encoding='utf-8')
                f_d = json.load(f)
                title = f_d["title"]
...
def content_ner():
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for i in range(302):
            ...
            for line in f.readlines():
                newslist.append(int(line))
            fout = codecs.open('content_ner/%d.json' % m, 'w', encoding='utf-8')
            for i in range(len(newslist)):
                day = newslist[i]
                f = codecs.open('D:/PycharmProjects/news_data/%d.json' % day, encoding='utf-8')
                f_d = json.load(f)
                ...
                result = model.evaluate_line(sess, input_from_line(content, char_to_id), id_to_tag)
                dicObj = json.dumps(result)
                fout.write(dicObj)
                fout.write("\n")
            fout.close()
